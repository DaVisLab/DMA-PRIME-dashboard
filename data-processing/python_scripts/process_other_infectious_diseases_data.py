...
with open(
    get_file_descriptor(
        f"{processed_data_dir}/other_infectious_diseases/metadata.json"
    ),
    "w",
) as f:
...
for region_size, file_region_size in region_file_identifiers.items():
    ...
    for column in other_column.keys():
        ...
        with open(
            ...
            state_aggregation = geojson.Feature(
                geometry=state_central_point, properties=state_properties
            )
            ...
            for feature in gj.features:
                try:
                    identifier = int(
                        feature.properties[region_geojson_identifiers[region_size]]
                    )
                except ValueError:
                    identifier = feature.properties[
                        region_geojson_identifiers[region_size]
                    ]
            ...
            for disease_dir in dirs:
                ...
                file = glob.glob(
                    f"{aggregated_data_dir}/other_diseases/{disease_dir}/*{file_region_size}*.csv"
                )[0]

                df = pd.read_csv(file).rename(
                    {
                        file_region_size: region_size,
                        "Week": "date",
                ...
                dates = pd.date_range(
                    ...
                    start=start_date,
                    freq=f"W-{pd.to_datetime(start_date).day_name()[:3].upper()}",
                )

                if other_column[column]:
                    df = pd.pivot_table(
                        df,
                        values=[column, other_column[column]],
                        index=[region_size, "date"],
                    )
                else:
                    df = pd.pivot_table(
                        df, values=[column], index=[region_size, "date"]
                    )
                ...
                gj["metadata"] = {"start_date": start_date, "end_date": end_date}
                ...
                for feature in gj.features:
                    ...
                    try:
                        feature_disease_data = pd.Series(
                            data=df.xs(identifier, axis=0)[column], index=dates
                        ).fillna(0)
                        ...
                        if padding:
                            reversed_data = pd.concat(
                                [reversed_data, pd.Series([np.nan] * padding)]
                            )
                        ...
                        state_aggregation.properties["data"][disease][
                            "weekly"
                        ] += feature_disease_data
                        state_aggregation.properties["data"][disease][
                            "monthly"
                        ] += feature.properties["data"][disease]["monthly"]
                        state_aggregation.properties["data"][disease][
                            "yearly"
                        ] += feature.properties["data"][disease]["yearly"]
                    ...
                    try:
                        feature_disease_other = pd.Series(
                            data=df.xs(identifier, axis=0)[other_column[column]],
                            index=dates,
                        ).fillna(0)
                        ...
                        state_aggregation.properties["other"][disease][
                            "weekly"
                        ] += feature_disease_other
                        state_aggregation.properties["other"][disease][
                            "monthly"
                        ] += feature.properties["other"][disease]["monthly"]
                        state_aggregation.properties["other"][disease][
                            "yearly"
                        ] += feature.properties["other"][disease]["yearly"]
                ...
                state_aggregation.properties["data"][disease]["weekly"] = (
                    state_aggregation["properties"]["data"][disease]["weekly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
                )
                state_aggregation.properties["other"][disease]["weekly"] = (
                    state_aggregation["properties"]["other"][disease]["weekly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
                )
                state_aggregation.properties["data"][disease]["monthly"] = (
                    state_aggregation["properties"]["data"][disease]["monthly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
                )
                state_aggregation.properties["other"][disease]["monthly"] = (
                    state_aggregation["properties"]["other"][disease]["monthly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
                )
                state_aggregation.properties["data"][disease]["yearly"] = (
                    state_aggregation["properties"]["data"][disease]["yearly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
                )
                state_aggregation.properties["other"][disease]["yearly"] = (
                    state_aggregation["properties"]["other"][disease]["yearly"]
                    .fillna(0)
                    .to_numpy()
                    .tolist()
