...
if os.path.exists(f"{scripts_supporting_files_dir}/token.json"):
    creds = Credentials.from_authorized_user_file(
        f"{scripts_supporting_files_dir}/token.json",
...
try:
    ...
    with open(
        get_file_descriptor(
            f"{aggregated_data_dir}/mhc/Clemson Rural Health Outreach.csv"
        ),
        "wb",
    ) as f:
        f.write(file.read())
...
df.rename(
    {
        ...
        "POC Contact Information": "POC_contact_info",
        "Number of patients (documented in Epic)": "num_epic_patients",
        "Number of People Attended (non-clinical)": "num_non-clinical_attendees",
        "Staff Members Present": "staff_members_present",
        "Site Address (If Different)": "site_address",
        "Report Submitted By:": "report_author",
        "Notes": "notes",
        "DMA Prime offered?": "DMAPRIME",
    },
    axis=1,
    inplace=True,
)
...
df.fillna(
    {
        ...
        "POC_contact_info": "",
        "num_epic_patients": 0,
        "staff_members_present": "",
        "num_non-clinical_attendees": 0,
        "report_author": "",
        "notes": "",
        "DMAPRIME": "no",
    },
...
try:
    address_translation = pd.read_csv(
        f"{scripts_supporting_files_dir}/mobile_health_clinic_address_translation.csv",
        index_col=0,
    )
except OSError:
...
address_translation.to_csv(
    f"{scripts_supporting_files_dir}/mobile_health_clinic_address_translation.csv"
)
with open(
    get_file_descriptor(
        f"{processed_data_dir}/mhc/clemson_rural_health_event_data.json"
    ),
    "w",
) as f:
