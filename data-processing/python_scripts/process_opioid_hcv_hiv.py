...
zcta_socio_demographics = pd.read_csv(
    f"{scripts_supporting_files_dir}/zcta_socio_demographics.csv", index_col=0
)
zcta_socio_demographics.rename(
    {"prop.Uninsured": "proportion_uninsured", "Median.Income": "median_income"},
    axis=1,
    inplace=True,
)
...
for disease, df in disease_dict.items():
    ...
    for year in years:
        temp = pd.DataFrame(
            df[["zcta", f"{disease}_hosp_{year}", f"{disease}_death_{year}"]].rename(
                {
                    f"{disease}_hosp_{year}": "hospitalizations",
                    f"{disease}_death_{year}": "deaths",
                },
                axis=1,
            )
        )
    ...
    pivot_df = pd.pivot_table(
        new_df,
        values=["hospitalizations", "deaths"],
        index=["zcta", "year"],
        dropna=False,
    )
    ...
    with open(
        f"{scripts_supporting_files_dir}/sc_zcta_population_simplified.json", "r"
    ) as f:
        gj = geojson.load(f)
                ...
                for year in years:
                    try:
                        ...
                        if math.isnan(value):
                            ...
                            if int(year) < 2023 or (
                                col == "hospitalizations" and int(year) == 2023
                            ):
                                zcta_disease_data[col][
                                    "cumulative"
                                ] += zcta_disease_data[col][year]
                    except KeyError:
                        zcta_disease_data[col][year] = "NaN"
        ...
        with open(
            get_file_descriptor(
                f"{processed_data_dir}/opioid_hcv_hiv/{disease}_zcta_hospitalization_data.json"
            ),
            "w",
        ) as f:
            geojson.dump(gj, f)
...
with open(
    get_file_descriptor(f"{processed_data_dir}/opioid_hcv_hiv/metadata.json"), "w"
) as f:
    metadata = {
