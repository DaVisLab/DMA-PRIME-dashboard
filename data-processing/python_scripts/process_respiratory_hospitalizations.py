...
outcome_variables_code_friendly = {
    k: "_".join(v.lower().split())
    for k, v in list(OrderedDict.fromkeys(outcome_variables.items()))
}
...
ds_to_pop = {"general_population": "RFA", "health_system": "HS"}
...
region_geojson_identifiers = (
    {  # identifier key for geographic unit within the geojson file
...
metadata = {
    ...
    "outcome_variables": {
        outcome_variables_code_friendly[k]: v for k, v in outcome_variables.items()
    },
    "outcome_variables_tooltips": {
        outcome_variables_code_friendly[k]: v[0] + v[1:].lower()
        for k, v in outcome_variables.items()
    },
    "available_models": {
        "covid_19": {
            "region": {
            ...
            "facility": {"health_system": ["positive_tests", "rate_of_transmission"]},
        },
        "influenza": {
            "region": {
                "general_population": ["all_hospitalizations"],
        ...
        "RSV": {
            "region": {"general_population": ["all_hospitalizations"]},
        },
        "respiratory_diseases": {
            "region": {"general_population": ["all_hospitalizations"]},
            "county": {"general_population": ["all_hospitalizations"]},
            "zcta": {"general_population": ["all_hospitalizations"]},
        },
    },
...
forecast_files = glob.glob(
    f"{aggregated_data_dir}/respiratory/**/*.csv", recursive=True
)
...
all_historical_dates = pd.date_range(
    end=current_date,
    start=first_date,
    freq=f"W-{day_of_week[:3].upper()}",
    inclusive="left",
).to_series()
short_history_dates = pd.date_range(
    end=current_date,
    start=start_short_history,
    freq=f"W-{day_of_week[:3].upper()}",
    inclusive="left",
).to_series()
...
def process_disease_location(data, disease_data, disease_data_all):
    for population in populations:  # for general and hs
        for (
            ...
            outcome_variables_code_friendly.values()
        ):  # for each encounter/inpatient/pos test/rt
            data_by_value_type = {}
            for value_type in e_p_r_crosswalk.values():
                try:
                    data_by_value_type[value_type] = data.xs(
                        (population, outcome_variable, value_type),
                        axis=0,
                        level=[
                            "population",
                            "outcome_measure",
            ...
            if (
                data_by_value_type["estimated"] is None
                or data_by_value_type["estimated"]["value"].isna().all()
            ):
                # no estimated, then reported -> historical

                if data_by_value_type["reported"] is not None:
                    try:
                        temp_data_values = data_by_value_type["reported"]["value"].xs(
                            ds_to_pop[population], axis=0, drop_level=True
                        )
                    except KeyError:
                        temp_data_values = pd.Series()

                    # short history
                    disease_data[population][outcome_variable]["historical"][
                        "values"
                    ] = pandas_to_json_safe_list(
                        temp_data_values.reindex(short_history_dates)
                    )
                    disease_data[population][outcome_variable]["historical"][
                        "reported"
                    ] = True
                    ...
                    # all history
                    disease_data_all[population][outcome_variable]["historical"][
                        "values"
                    ] = pandas_to_json_safe_list(
                        temp_data_values.reindex(all_historical_dates)
                    )
                    disease_data_all[population][outcome_variable]["historical"][
                        "reported"
                    ] = True
                ...
                temp_data_values = data_by_value_type["estimated"]["value"].droplevel(
                    "data_source"
                )
                temp_data_imputations = data_by_value_type["estimated"][
                    "imputed"
                ].droplevel("data_source")

                # short history
                disease_data[population][outcome_variable]["historical"]["values"] = (
                    pandas_to_json_safe_list(
                        temp_data_values.reindex(
                            short_history_dates, level="target_end_date"
                        )
                    )
                )
                disease_data[population][outcome_variable]["historical"]["imputed"] = (
                    bool(
                        temp_data_imputations.reindex(
                            short_history_dates, level="target_end_date"
                        ).any()
                    )
                )
                ...
                disease_data_all[population][outcome_variable]["historical"][
                    ...
                    temp_data_values.reindex(
                        all_historical_dates, level="target_end_date"
                    )
                )
                disease_data_all[population][outcome_variable]["historical"][
                    "imputed"
                ...
                if outcome_variable in [
                    ...
                    for data_source, data_source_code_friendly in data_sources.items():
                        try:
                            extra_data = data_by_value_type["reported"]["value"].xs(
                                data_source, axis=0, level="data_source"
                            )
                            if (
                                extra_data.reindex(
                                    short_history_dates, level="target_end_date"
                                ...
                                # short history
                                disease_data[population][outcome_variable]["extra"][
                                    data_source_code_friendly
                                ] = pandas_to_json_safe_list(
                                    extra_data.reindex(
                                        short_history_dates, level="target_end_date"
                                    )
                            ...
                            if (
                                ...
                                # all history
                                disease_data_all[population][outcome_variable]["extra"][
                                    data_source_code_friendly
                                ] = pandas_to_json_safe_list(
                                    extra_data.reindex(
                                        all_historical_dates, level="target_end_date"
            ...
            # projected
            if (
                data_by_value_type["projected"] is not None
                and data_by_value_type["projected"]["value"].notna().any()
            ):
                disease_data[population][outcome_variable]["projected"]["imputed"] = (
                    bool(data_by_value_type["projected"]["imputed"].any())
                )
                disease_data[population][outcome_variable]["projected"][
                    "start_date"
                ] = (
                    data_by_value_type["projected"]["value"]
                    .index.get_level_values("target_end_date")
                    .min()
                    .strftime("%Y-%m-%d")
                )
                disease_data[population][outcome_variable]["projected"]["values"] = (
                    pandas_to_json_safe_list(data_by_value_type["projected"]["value"])
                )

                disease_data_all[population][outcome_variable]["projected"][
                    "imputed"
                ] = bool(data_by_value_type["projected"]["imputed"].any())
                disease_data_all[population][outcome_variable]["projected"][
                    "start_date"
...
def get_facility_data(facility, extended_historical_data):
    ...
    disease_data = {
        population: {
            var: {
                "historical": {"imputed": 0, "reported": 0, "values": []},
                "projected": {
                    "imputed": 0,
...
facility_info = pd.read_csv(
    f"{scripts_supporting_files_dir}/Cross-WALK-LOCATIONS-temp.csv"
)
extended_historical_data = {}

print("facility")
for disease in diseases.keys():
    all_disease_data = geojson.FeatureCollection(
        facility_info.apply(
            get_facility_data, args=[extended_historical_data], axis=1
        ).to_list()
    )
...
for region_size, identifier_column in region_geojson_identifiers.items():
    ...
    with open(
        f"{scripts_supporting_files_dir}/sc_{region_size}_population_simplified.json",
        "r",
    ) as f:
        gj = geojson.load(f)
        ...
        if (
            region_size == "state"
        ):  # fixing for state or won't be able to grab data from df
...
if len(csvs) > 0:
    ...
    state_cdc["Week.Ending.Date"] = pd.to_datetime(
        state_cdc["Week.Ending.Date"], format=date_format
    )

    state_cdc = state_cdc[
        (start_short_history <= state_cdc["Week.Ending.Date"])
        & (state_cdc["Week.Ending.Date"] <= current_date)
    ]
    ...
    for disease in diseases.keys():
        ...
        column = list(
            filter(
                lambda x: f'total.{".".join(disease.lower().split("_"))}.admissions'
                == x.lower(),
                state_cdc.columns,
            )
        )
...
######## 5 ########
with open(
    get_file_descriptor(f"{processed_data_dir}/respiratory/metadata.json"), "w"
) as f:
    ...
    metadata["all_historical_dates"] = all_historical_dates.dt.strftime(
        "%Y-%m-%d"
    ).to_list()
    metadata["start_short_history"] = start_short_history.strftime("%Y-%m-%d")
    metadata["short_history_dates"] = short_history_dates.dt.strftime(
        "%Y-%m-%d"
    ).to_list()
