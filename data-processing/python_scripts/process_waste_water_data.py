...
def clean_string(string):
    new_string = string.replace("\xc2", "")
    new_string = string.replace("\xa0", "")
    return new_string


disease_names = {"InfA": "Influenza A", "SARS": "SARS-CoV-2", "RSV": "RSV"}
...
with open(f"{aggregated_data_dir}/waste_water/site_info.json", "r") as f:
    ...
    for site, info in site_info.items():
        ...
        with open(
            get_file_descriptor(f"{processed_data_dir}/waste_water/{site}.json"), "w"
        ) as f:
            json.dump(data_dict, f)

    with open(
        get_file_descriptor(f"{processed_data_dir}/waste_water/metadata.json"), "w"
    ) as f:
        metadata = {
            "site_info": site_info,
