...
if len(sys.argv) != 3:
    print(
        "Usage: python3 data_approval_email_notifications.py datetime checkpoint_file"
    )
    sys.exit(1)
...
recipients = ["ambleic@clemson.edu", "eserman@clemson.edu"]
    ...
    if current_datetime.hour in [9, 12, 15]:
        try:
            ...
            if len(dashboards):
                message = "\n".join(
                    [
                        f"Report of data processing - {bash_datetime}",
