...
if [ $? -ne 0 ]; then # error during processing
    ...
    diff -r --brief $latest_backup/aggregated/mhc $visualization_data_dir/aggregated/mhc # compare
...
diff -r --brief $latest_backup/aggregated/opioid_hcv_hiv $visualization_data_dir/aggregated/opioid_hcv_hiv
if [[ $? -ne 0 ]]; then # new data!
    # process data
...
diff -r --brief $latest_backup/aggregated/respiratory $visualization_data_dir/aggregated/respiratory &> respiratory_changes.txt
if [[ $? -ne 0 ]]; then # new data!
    # process data
...
diff -r --brief $latest_backup/aggregated/waste_water $visualization_data_dir/aggregated/waste_water
if [[ $? -ne 0 ]]; then # new data!
    # process data
