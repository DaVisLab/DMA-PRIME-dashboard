...
function updateDiseaseCountDisplay() {
  ...
  diseases.forEach((disease) => {
    ...
    let prevVal =
      stateFeature.properties.data[disease][mapTimeSwitch.value].at(-2) || 0;
...
function getData(feature, timeFrame = "weekly") {
  ...
  if (diseases.length > 0) {
    ...
    if (dataDicts.length > 0) {
      switch (timeFrame) {
        ...
        case "yearly":
          periods = Math.ceil(
            d3.timeDay.count(earliestDate, latestDate) / (52 * 7)
          );
          break;
      }
  ...
  if (mapRateSwitch.value == "rate") {
    thisData.data = thisData.data.map(
      (val) => parseFloat(val) / (thisData.population / 1000.0) || 0
    );
