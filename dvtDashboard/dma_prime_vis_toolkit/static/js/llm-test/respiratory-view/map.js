...
} from "/static/js/llm-test/respiratory-view/script.js";

import {
  unknownColor,
  populationColorMap,
} from "/static/js/llm-test/respiratory-view/utils/colors.js";

import {
  drawTooltip,
  showSimpleGeoTooltip,
} from "/static/js/llm-test/respiratory-view/tooltip.js";

import {
  facility_dataProcessing,
  call_data,
  getCurDateValueFromFeature,
} from "/static/js/llm-test/respiratory-view/utils/dataProcessing_utils.js";

import {
  applyRespiratoryOptionRestrictions,
  getCurrentControlState,
  resolveRespiratoryControlState,
} from "/static/js/llm-test/respiratory-view/utils/controlState_utils.js";
...
} from "/static/js/llm-test/respiratory-view/utils/interfaceOption_utils.js";

const { GeoJsonLayer, IconLayer, TextLayer, MapboxOverlay } = deck;
const respiratoryMapRoot = document.getElementById("respiratory-map-panel");
const respiratoryMapDiv =
  respiratoryMapRoot?.querySelector("#map-div") || mapDiv;
...
d3.select(respiratoryMapRoot).selectAll(".map-option").attr("disabled", null);
...
document.addEventListener("sl-tab-show", (event) => {
  if (event.detail?.name !== "respiratory-map") return;
