...
export function dehighlightSymbol(map, layer, ROIs) {
  // map.setPaintProperty(layer, "icon-halo-color", [
  //   "case",
  //   ["in", ["get", "id"], ["literal", ROIs]],
  //   "#cccccc",
...
export async function getSpatialData(space_resolution = "region") {
  ...
  return regionData.features.map((d) => {
    if (mapSpatialResoultion == "state") {
      ...
      d.properties.id = d.properties.display_name;
      d.properties.name = d.properties.display_name;
    }

    d.properties.id = d.properties.id.toLowerCase().replaceAll("-", " ").replaceAll(" ", "_");
    ...
    d.properties.projected_value =
      d.properties.projected_value == undefined
        ? 0
        : d.properties.projected_value;
    return d;
  });
...
export function fillAreaGeoJSONLayer(map, sourceID, layerID, paintOptions) {
  ...
  map.once("error", (e) => {
    ...
    // Add fallback layer - error handling when data is missing
    map.addLayer({
      id: layerID,
      type: "fill",
      source: sourceID,
      paint: {
        "fill-color": "#cccccc",
        "fill-opacity": paintOptions["fill-opacity"] ?? 1,
      },
    });
  });
...
export function targetMapsAndLayersByCurrentSpatialResolution() {
  return {
    mapSpatialResoultion: document.getElementById(
      "map-resolution-selector"
    ).value,
    targetMap: maps.baseMap,
    targetLayer: maps.layers,
  };
}
...
export function addMapColorSchemeInfo(thisSpatialResolution, colorScale) {
  ...
  let svg = d3.select("#focus-map-legend-svg");
