...
async function updateData() {
  ...
  gridContainerD3
    ...
    .each(function (feature) {
      ...
      var gridTTPContainer = gridItemContainer
        .append("sl-tooltip")
      ...
      gridTTPContainer.node().updateComplete.then(function (a, b) {
        var slTtpBody = d3
          .select(gridTTPContainer.node().shadowRoot)
        ...
        gridTTP
          ...
          .attr(
            "class",
            "grid-close-tooltip-button grid-tooltip-toolbar-button",
          )
        ...
        gridTTP
          ...
          .attr(
            "class",
            "grid-open-expanded-tooltip-button grid-tooltip-toolbar-button",
          )
          .on("click", () => {
            ...
            tooltipLarge.show().then(async () => {
              var allExtendedData = await d3.json(
                `/data/respiratory/${gridGeographicUnitSelector.value}/${gridDiseaseSelector.value}/extended?data_version=${metadata.data_version}&${parseInt(Math.random() * 9999999999)}`,
              );
              var ttpData = {
                id: location,
        ...
        gridTTP
          ...
          .attr(
            "class",
            "grid-tooltip-toolbar-button grid-model-exploration-icon-button",
          )
          .on("click", () => {
            window.open(
      ...
      var gridTTPHeader = gridTTP.append("div").attr("class", "tooltip-header");
      gridTTPHeader.append("div").attr("class", "tooltip-region-info");
      gridTTPHeader.append("div").attr("class", "tooltip-data-info");
      gridTTP
        .append("svg") // tooltip graph in svg
        .attr("id", `grid-${idLocation}-tooltip-svg`)
        .attr("class", `tooltip-outer-svg`);
      var gridTTPFooter = gridTTP.append("div").attr("class", "tooltip-footer");
      gridTTPFooter.append("div").attr("class", "tooltip-legend");
      gridTTPFooter.append("div").attr("class", "tooltip-options");

      // main visualization
      var gridDiv = gridTTPContainer
        .append("div")
      ...
      gridDiv.on(
        "click",
        () => (gridTTPContainer.node().open = !gridTTPContainer.node().open),
      );
      ...
      var historicalLine = gridSVG
        .append("path")
        .attr("class", "grid-item-historical-line");

      // add value label and dot
      var valueLabel = gridSVG.append("g").attr("class", "grid-item-value");
...
async function updateGrid(fetchData = true) {
  ...
  // redo summary text
  let outcomeVariableString = d3
    .select(gridOutcomeVariableSelector)
    .select(`*[value="${gridOutcomeVariableSelector.value}"]`)
    .html();
  ...
  gridSummaryText += d3
    .select(gridDiseaseSelector)
    .select(`*[value="${gridDiseaseSelector.value}"]`)
    .html();
  ...
  gridSummaryText += d3
    .select(gridGeographicUnitSelector)
    .select(`*[value="${gridGeographicUnitSelector.value}"]`)
    .html()
    .toLowerCase()
    .replace("zcta", "ZCTA");
  gridSummaryText += `.`;
  ...
  var colItems = Math.min(
    6,
    Math.max(Math.floor(adjustedHeight / (120 - 0.25 * em)), 1),
  );
  var rowItems = Math.min(
    8,
    Math.max(Math.floor(adjustedWidth / (150 - 0.25 * em)), 1),
  );

  if (
    d3.select(gridContainer).selectAll("div.grid-container").size() >
    colItems * rowItems
  ) {
    ...
    gridColor = d3
      ...
      .range(
        d3.quantize(
          d3.interpolateRgb(
            "white",
            populationColorMap[gridPopulationSelector.value]["historical"],
          ),
          5,
        ),
      )
  ...
  gridContainerD3.selectAll("div.grid-container").each(function (feature) {
    ...
    historicalGroup.attr(
      ...
      d3
        ...
        data.historical.values.concat([data.projected.values[0]]),
      ),
    );
    ...
    if (displayValue) {
      ...
      lastValueMarker.select("circle").attr("cx", 0).attr("cy", 0);

      lastValueMarker.select("line").attr("display", "none");
    } else {
...
function setupGridTooltip(ttpDiv, redraw = false) {
  ...
  var gridTooltipWidth = Math.max(500, gridWidth * 0.3);
  var gridTooltipHeight = gridTooltipWidth * 0.65;
  ...
  drawTooltip(
    thisData,
    ttpSVG,
    slTTP.select(".tooltip-header"),
    slTTP.select(".tooltip-footer"),
    gridPopulationSelector.value,
    gridOutcomeVariableSelector.value,
    gridTypeSwitch.value,
    true,
