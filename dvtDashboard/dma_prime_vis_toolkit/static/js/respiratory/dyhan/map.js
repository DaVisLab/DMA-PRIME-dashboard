...
async function getSpatialData() {
  const mapSpatialResoultion = "region";
  ...
  //   let returnValue =  regionData.features.map((d) => {
...
function initMap(map, divComponent) {
...
map.on("load", async () => {
  ...
  initMap(map, document.getElementById("state-map-div"));
  const featuresDataBySpace = await getSpatialData();
  ...
  function selectLine(map, layer, lineId) {
  ...
  map.on("click", "my-geojson-layer", function (e) {
  ...
  map.on("mousemove", "my-geojson-layer", function (e) {
    // Access the clicked feature's data
    const features = e.features[0];
    const coordinates = features.geometry.coordinates;
    const properties = features.properties;
    ...
    selectLine(map, "my-geojson-layer2", features.properties.id);
    // // Create and add a popup
    // new maplibregl.Popup()
    //     .setLngLat(coordinates)
    //     .setHTML('<h3>' + properties.name + '</h3>' + properties.description)
