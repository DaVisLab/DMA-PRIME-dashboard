...
def add_user():
    if request.method == "POST":
        ...
        try:
            ...
            if existing_user:
                current_app.logger.info(
                    f"{current_user.email} failed to create user {email} (user already exists)"
                )
    ...
    if request.method == "POST":
        ...
        try:
            ...
            if not existing_user:
                ...
                current_app.logger.info(
                    f"{current_user.email} failed to delete user {email} (user does not exist)"
                )
                return redirect("/admin")
            ...
            current_app.logger.info(
                f"{current_user.email} failed to delete user {email} (error)"
            )
            flash(str(e))
            return redirect("/admin")
    ...
    if request.method == "POST":
        ...
        try:
            if field == "username":
                ...
                db.session.commit()
                current_app.logger.info(
                    f"{current_user.email} changed access_level of {email}"
                )
            ...
            current_app.logger.info(
                f"{current_user.email} attempted to change {field} of {email}"
            )
            flash(str(e))
            return redirect("/admin")
